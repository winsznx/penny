...
/**
 * Spreadsheet-friendly serialiser for the leaderboard "export" button. Quotes
 * any cell with commas or quotes so the result re-imports cleanly.
 */
export function rowsToCsv(rows: LeaderboardRow[], events: ActorEvent[]): string {
  const eventNames = events.map((e) => e.name);
  const headers = ["rank", "address", "actions", "cusd_moved", "last_block", ...eventNames];

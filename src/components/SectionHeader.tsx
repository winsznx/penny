  ...
  return (
    <header className={`mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between ${className}`}>
      <div className="min-w-0">
        {eyebrow && (
          <div className="font-mono text-[11px] uppercase tracking-[0.15em] text-stone-text">
            {eyebrow}
          </div>
        )}
        <h2 className="font-display text-2xl font-bold tracking-tight text-midnight">{title}</h2>
        {description && <p className="mt-1 max-w-prose text-sm text-stone-text">{description}</p>}
      </div>

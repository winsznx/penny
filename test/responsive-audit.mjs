...
const LONG_WORD = "PENNY_RESPONSIVE_AUDIT_UNBREAKABLE_ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
...
function mockHiroTransactions() {
  return {
    ...
    results: Array.from({ length: 18 }, (_, i) => ({
      ...
      contract_call: {
        contract_id: "SP31DP8F8CF2GXSZBHHHK5J6Y061744E1TNFGYWYV.penny",
        function_name: i % 3 === 0 ? `register-message-${LONG_WORD}` : "register-message",
      },
    })),
  };
}
...
async function seedWorstCaseStorage(context, chain) {
  await context.addInitScript(
    ({ chainKind, longWord }) => {
      window.localStorage.setItem("penny_chain_kind", chainKind);
      window.localStorage.setItem("penny_audit_long_word", longWord);
      window.localStorage.setItem("penny_highscore", "987654321987654321");
      window.localStorage.setItem(
        ...
        JSON.stringify(
          Array.from({ length: 20 }, (_, i) => ({
            score: 987654321 + i,
            coins: 123456789 + i,
            difficulty: i % 2 === 0 ? `beast-${longWord}` : `normal-${longWord}`,
      ...
      for (const viewport of viewports) {
        ...
        try {
          ...
          if (viewport.width < 768) {
            const menu = page.locator("[aria-controls='flap-mobile-menu']").first();
